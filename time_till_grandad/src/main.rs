...
fn message(duration: &Duration) -> &'static str {
    ...
    if duration.num_days() <= 0 {
        let out = there_message = ["You are there, have fun!", "Why don't you go on a walk", "Show John this program", "Stop staring at your computer", "Did you buy those shoes"]
    }
    else if duration.num_hours() <= 1 {
        let out = hour_message[rand::thread_rng().gen_range(0..day_message.len())];
        out
    }
